<script lang="ts">
	...
	let confirmPassword = field("confirm_password", "", [
		required(),
	...
	if (browser) {
		posthog.onFeatureFlags((flags) => {
			dynamic_features.feature_delete_account =
				posthog.isFeatureEnabled("delete_account") ?? false;
		});
	}
		...
		<div class="flex flex-col gap-9">
			...
			>
				...
				<div class="flex flex-col items-center lg:items-start gap-3">
					<span
						class="text-3xl font-medium capitalize text-slate-600"
					>
					...
					<div class="flex justify-center gap-6">
						<div class="flex items-center">
							<ProfileIcon
								class="h-4 w-4 text-slate-500 fill-current"
							/>
				...
				<div use:melt={$content("tab-1")} class="grow bg-white p-5">
					...
					>
						<div class="flex w-full flex-col gap-1">
							<span
								class="mx-1 text-lg font-medium text-slate-700"
								>Email Address</span
							>
							<p class=" max-w-md text-xs text-slate-500">
								This will require for you to re-verify your
								Email
							</p>
							<Textfield
								type="email"
					...
					>
						...
						>
							<div class="flex flex-col gap-1">
								...
								<p class=" max-w-md text-xs text-slate-500">
									Make sure that your password is longer than
									8 characters and contains uppercase,
							...
							{#if $page.form?.formErrors && $page.form?.formErrors.length}
								<div class="w-full">
									<span
										class="text-xs font-semibold capitalize text-red-600"
									>
							...
							>
								<ButtonSolid
									...
									>Save</ButtonSolid
								>
								<span class="pt-3 text-emerald-600"
									>{resetPwdResponse}</span
								>
							</div>
						</div>
					</form>
					...
					>
						<div class="flex flex-col gap-1">
							...
							<span class="text-slate-600 antialiased text-sm">
								This information is public and visible to all
								users of OpenRegistry
							</span>
						</div>
						<div
							class="flex w-full flex-col items-start justify-start gap-3"
					...
					{#if dynamic_features.feature_delete_account}
						...
						>
							<div class="flex flex-col gap-1">
								<span
									class="text-lg font-medium text-rose-700/80"
									>Delete Account</span
								>
								<span
									class="text-slate-600 antialiased text-sm max-w-md"
								>
									Please note, if you choose to delete your
									account your repositories will also deleted
									and will no longer be available to you or
									anyone else
								</span>
							</div>
							<div
