...
<div class="flex dark:bg-brown-900">
	...
	{#each new Array(pages).fill(0) as i,page}
		...
		<button
				...
				class=" {activePage === page ? 'bg-brown-400' : 'bg-cream-50'} px-4 py-2 mx-1 text-gray-700 transition-colors duration-200 transform rounded-md sm:inline
		 		hover:bg-gray-200 hover:text-gray-700 border-brown-100"
		>
			{page+1}
		</button>
	{/each}
