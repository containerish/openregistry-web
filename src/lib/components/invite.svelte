...
<div use:melt={$portalled} class="w-full h-full">
  {#if $open}
    ...
    >
      ...
      >
        <span
          class="text-2xl capitalize font-semibold text-primary-500"
          use:melt={$title}>Invite Your Colleauges</span
        >
      ...
      >
        <div class="w-full flex flex-col gap-2 h-full">
          <div class="flex flex-col w-full gap-3">
            <span class="text-slate-500 text-sm"
              >fill in a comma separated list of emails below and hit send to
          ...
          {#if respMsg}
            <div class="w-full text-center">
              <span
                class={statusCode > 299 ? "text-rose-600" : "text-slate-700"}
                >{respMsg}</span
              >
            </div>
          {/if}
        ...
        <div class="flex w-full gap-6 justify-center">
          ...
          <ButtonSolid class="w-full" {isLoading} on:click={sendInvites}
