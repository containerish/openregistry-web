...
{:else}
	<div
		...
		px-8 py-2 desktop:py-1.5 my-2 mx-2 bg-slate-50 border-2 border-primary-100 rounded-sm"
	>
		<div class="pt-2 text-xl desktop:text-[16px] text-slate-700">
			<span>
				{data.namespace.split('/')[0]} /
		...
		<div class="flex items-center justify-between">
			<span class="text-sm desktop:text-xs font-light text-gray-400">
				{new Date(data.created_at).toDateString()}
			</span>

			<div class="flex gap-4">
				<DownloadIcon styles="text-primary-500 h-6 w-6 desktop:h-5 desktop:w-5" />
				<StarIcon styles="text-primary-500 h-6 w-6 desktop:h-5 desktop:w-5" />
				<LockOpenIcon styles="text-primary-500 h-6 w-6 desktop:h-5 desktop:w-5" />
			</div>
		</div>
