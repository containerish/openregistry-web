...
<div use:melt={$portalled} class="w-full h-full">
  {#if $open}
    ...
    <div
      ...
      fixed left-[50%] bg-white top-[50%] z-50 max-h-[30vh] w-[36vw] max-w-[200px]
     translate-x-[-50%] translate-y-[-50%] shadow-3xl shadow-primary-800/40 gap-2"
      use:melt={$content}
    >
      <!-- <Logo type="dark" /> -->
      <div
        class="flex w-full flex-col items-center justify-center gap-3 text-center"
      >
        <div
          class="flex flex-col items-center gap-3 bg-slate-300/50 px-3 py-6 rounded"
        >
          <CrossIcon class="h-16 w-16 text-rose-700/80" />
          <p
            class="font-semibold capitalize tracking-wider text-rose-700 max-w-lg"
          >
            {error}
          </p>
        </div>
        ...
        <p
          class="text-xs font-semibold capitalize tracking-wider text-gray-600 max-w-sm"
        >
      ...
      <div class="mt-8 flex w-full items-center justify-center">
        <ButtonOutlined on:click={() => closeModal()}>
          <button class="p-0 m-0 w-full h-full" use:melt={$close}>
            Close
          </button>
        </ButtonOutlined>
      </div>
