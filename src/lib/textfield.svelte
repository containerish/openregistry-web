...
<div class="flex items-center px-2">
	<label for={label} class="block font-semibold text-sm -ml-2 text-slate-700"
		>{label}</label
	>
	{#if subHeading !== ''}
...
<input
	...
	{disabled}
	class={twMerge(
		`placeholder-slate-500 form-control block w-full px-3 py-3 h-11 text-sm font-normal
